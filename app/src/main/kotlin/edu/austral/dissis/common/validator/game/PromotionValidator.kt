...
class PromotionValidator(
    ...
    override fun validate(movementData: MovementData, game: Game): ValidatorResult {
        if (isPieceInLimit(movementData, game, pieceFrom)) {
    ...
    private fun isPieceInLimit(movementData: MovementData, game: Game, pieceFrom: PieceType): Boolean {
        val piece = movementData.piece!!
        val board = game.board
        val limit = if (piece.color == ColorEnum.WHITE) board.height else 1
        return movementData.squareTo.y == limit && piece.name == pieceFrom
    }
}